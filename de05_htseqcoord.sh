...
#$ -o /user/logs/de_coordhtseq.$JOB_ID.$TASK_ID.out
#$ -e /user/logs/de_coordhtseq.$JOB_ID.$TASK_ID.err
...
#echo "htseq-count -f bam -r pos -s reverse -a 4 -i gene_id -m union ${bamOutput}/${FILENAME[$SGE_TASK_ID-1]}.sorted.bam $GTF > ${bamOutput}/${FILENAME[$SGE_TASK_ID-1]}.coord_counts"
#htseq-count -f bam -r pos -s reverse -a 4 -i gene_id -m union \
#  "${bamOutput}/${FILENAME[$SGE_TASK_ID-1]}.sorted.bam" \
#  $GTF \
#  > ${htseqOutput}/${FILENAME[$SGE_TASK_ID-1]}.coord.counts

echo "htseq-count -f bam -r pos -s reverse -a 4 -i gene_id -m union ${twopass}/${FILENAME[$SGE_TASK_ID-1]}.sorted.bam $GTF > ${twopass}/${FILENAME[$SGE_TASK_ID-1]}.coord_counts"
htseq-count -f bam -r pos -s reverse -a 4 -i gene_id -m union \
